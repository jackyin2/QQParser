...
class SimpleCalendar {
  ...
  create() {
    ...
    for (var i = 0; i < 42; i++) {
      days.innerHTML = days.innerHTML + '<div class="sc-item"><div class="day"></div><div class="black">————</div><div class="tongji"></div><div class="lunar-day"></div></div>';
    }
    //添加下拉框数据
  ...
  update(month = this.tmonth, year = this.tyear) {
    this.updateSize();
    this.updateWeek();
    this.addData(year, month);    //2018. 6
  ...
  addData(year, month) {
    ...
    for (var i = 0; i < 42; i++) {
      ...
      // 非本月的时间日期，增加样式othermonth区别
      if (writemonth != month) {
        daysElement[i].classList.add('sc-othermonth');
      }
      daysElement[i].querySelector('.day').innerHTML =writemonth.toString()+'-'+writeday.toString() ;
