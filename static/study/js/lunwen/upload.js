    ...
    _this.uploader.on('error', function (type) {
        switch (type) {
            case 'Q_EXCEED_NUM_LIMIT':
                $M().msg("学生一次只能上传一篇论文！").position('10%', '50%').time(3);
...
upload.prototype.saveMetaData = function () {
    ...
    var remark = $('#upload-form').find(".remark-text").val();
    _.ajax(urls.doc_upLoadFile, { data: JSON.stringify(_this.docs), meta: JSON.stringify(_this.meta), declareId: _.hash.Id, remark:remark}, function (r) {
        if (!r) {
            if (_this.onError && $.isFunction(_this.onError)) {
