            ...
            switch (opts.data.category) {
                case 'img':
                    ...
                    if (opts.data.extension == 'gif') {
                        ...
                        $._loadJs(opts.baseUrl + 'viewer/imageView/jquery.iviewer.min.js', function () {
                            ele.iviewer({
                                src: opts.data.extension == 'dwg' ? $.insertString(url, url.indexOf('?'), '/thumbnail.png') :url,
                ...
                case 'video':
                    ...
                    if (navigator.appName == "Microsoft Internet Explorer" && (navigator.appVersion.split(";")[1].replace(/[ ]/g, "") == "MSIE6.0" || navigator.appVersion.split(";")[1].replace(/[ ]/g, "") == "MSIE7.0" || navigator.appVersion.split(";")[1].replace(/[ ]/g, "") == "MSIE8.0" || navigator.appVersion.split(";")[1].replace(/[ ]/g, "") == "MSIE9.0")) {
                        jwpjs = "viewer/jwplayer/jwplayernew.js";

                    } else {
                        jwpjs = "viewer/jwplayer/jwplayer.js";
                    }

                    $._loadJs(opts.baseUrl + jwpjs, function () {
                        ...
                        var player = jwplayer(ele.find('.video_container').attr('id')).setup({
                            flashplayer: opts.baseUrl + 'viewer/jwplayer/player.swf',
                            levels: source,
                            autostart: true,
                            provider: 'http',
                            startparam: 'start',
                            ...
                            case 'ods':
                                $._loadJs(opts.baseUrl + 'viewer/MPreview/MPreview.min.js', function () {
                                    ...
                                    for (var i = 1; i <= opts.data.args.page_count; i++) {
                                        imgs.push($.insertString(opts.data.urls.preview, opts.data.urls.preview.indexOf('?'), '/' + i + '.png'));
                                    }
